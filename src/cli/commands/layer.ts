...
addApplyCommandOptions(
  pluginCmd
    ...
    .argument(
      "[plugins...]",
      "Plugin name(s), plugin export path, or URL (multiple roots become an ephemeral composition)",
    )
    .description(
      "Apply one or more plugins (or a plugin export URL) to a project, serializing for each harness",
    ),
).action(async (plugins: string[], opts: ApplyCommandOpts) => {
  await handleApplyCommand(plugins as [string, ...string[]] | [], opts);
});
