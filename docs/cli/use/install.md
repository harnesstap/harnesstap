...
That reads repo-root `apm.yml`, resolves `dependencies.apm` / `dependencies.mcp`, compiles local `.apm/` primitives, writes `apm.lock.yaml`, and materializes the **resolved** target harness directories (`.claude/`, `.cursor/`, `AGENTS.md`, and so on). Pin `targets:` so the written set does not follow whichever tool folders exist on the current machine. Preview with `ht targets`. [`ht compile`](./compile.md) is the same apply-from-manifest loop under a named entry.
