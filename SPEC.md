...
`apply` resolves local plugin names, bundle paths (directory or `.zip`), and URLs. With no selector — the same path as `ht install` — it compiles local `.apm/` primitives into harness directories (existing writers; `--target` / `--all` / `--harness`, then `targets` / `target` / `compilation.target`, then APM filesystem signals). Install and apply-from-manifest fail closed when no target resolves. Packed bundles with `pack.bundle_files` are rehashed and fail closed on mismatch, extra, missing, or symlink. Subsequent apply of a project verifies `local_deployed_file_hashes` the same way unless `--update` is passed. Critical hidden Unicode in generated files blocks apply unless `--force` is passed. Published selectors (`org/catalog/name@version` or `org/name@version`) that are not installed locally are fetched from the catalog at apply time (same import path as `plugin pull`). `ht compile` runs the same overlay writers without resolving dependencies or rewriting the lockfile. `ht targets` previews the resolved set.
