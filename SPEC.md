...
| `harnesstap mcp …` | Official MCP Registry v0.1 discovery and install. `mcp search` / `mcp list` / `mcp show` call `registry.modelcontextprotocol.io`. `mcp install <id>` is an alias of `ht install --mcp <id>`: append to `apm.yml` `dependencies.mcp`, then the existing apply-from-manifest install. Failed install rolls back the manifest write. Does not walk `dependencies.apm` of fetched packages, does not inject GitHub PATs, and does not add harness adapters HT does not already serialize. |
