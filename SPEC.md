...
| `resource show` | Prints the resource with display labels. Origin is `Local` / `Local (/path)` (or Marketplace). Path is the package directory for plugins and `SKILL.md` packages. Plugin content lists nested install files. Supports selector grammar. |
