...
- `project config`: optional `{project}/apm.yml` declaring named profiles (local / catalog / inline), environments, and plugin composition for `ht use` / `ht install` / `ht apply`. HarnessTap-only fields are first-class top-level keys (`default_profile`, `environments`, `profiles`, `plugins`). `environments` is a mapping: `default` names the active environment; other keys are named environments with `values` and optional `secret_refs`.
...
| `harnesstap apply` | Resolves a plugin dependency graph and materializes it into a project, or into machine home with `--global`. With no plugin selector, same loop as `ht install`: resolves the graph from `apm.yml`, including git `dependencies.apm` entries (shorthand `owner/repo`, https/ssh/`git@` URLs, optional `path` and `ref` / `#version`). Each git dep resolves to an exact commit SHA (fail closed), is fetched, and is recorded in `apm.lock.yaml` (`repo_url`, `resolved_commit`, ref/constraint, `path` if any, plus content hashes of deployed files). After a git, path, or catalog APM package is fetched, apply walks that package’s own `dependencies.apm` (not nested `devDependencies`) and keeps installing until the graph is complete; each walked package is pinned in `apm.lock.yaml` and deployed through the same overlay / harness writers. Cycles, missing packages, and lock hash mismatches fail closed. When two packages share a name, the root manifest wins. A later apply without `--update` replays the lock. Local `.apm/` primitives (`agents`, `skills`, `commands`, `hooks`, plus `instructions` / `prompts`) are compiled into the resolved target harness directories via the existing writers; `.apm/` wins over root primitive dirs (same skip warning as `ht pack`). Target resolution is `--target` / `--all` / `--harness`, then `targets` / `target` / `compilation.target` in `apm.yml`, then project/global harness preference, then filesystem auto-detect. Declared `targets:` wins over preference and folder detection. Apply-from-manifest, `ht install`, and `ht compile` fail closed when no target resolves after that chain. Also applies a local Agent Plugins bundle directory or `.zip` (rehashing `pack.bundle_files` and failing closed on mismatch, extra, missing, or symlink). Generated files are scanned for hidden Unicode before write; critical findings block apply unless `--force` is passed. When `apm.lock.yaml` already records `local_deployed_file_hashes`, apply verifies SHA-256 and fails closed on mismatch, extra, or missing unless `--update` is passed. `apm-policy.yml` (and a pinned `policy.hash` on `apm.yml`) is evaluated against the install plan before any harness byte is written. When the project declares `executables:` or the policy has a non-empty `executables:` block, unapproved executable primitives from dependency packages (hooks, `bin/`, self-defined MCP) are parked and the lock records `exec_status`; apply still succeeds and prints `ht approve <ref>`. Local `.apm/` and the apply root stay trusted. Text primitives are never gated. |
| `harnesstap install` | Project onboarding alias of `apply` with no plugin selector. Reads repo-root `apm.yml`, resolves `dependencies.apm` (including transitive `dependencies.apm` on fetched git / catalog / path packages) / `dependencies.mcp` (including MCP Registry v0.1 identities such as `io.github…`), compiles local `.apm/`, writes `apm.lock.yaml`, and materializes the resolved target harness directories. Registry identities fetch `GET /v0.1/servers/{id}/versions/latest` from `registry.modelcontextprotocol.io` and emit the same native MCP files HT already serializes. Self-defined `registry: false` stays inline. `--mcp <id>` appends the identity to `apm.yml` then runs install; a failed install rolls the manifest write back. Same target resolution as `ht compile`. Fails closed when no target can be resolved (declared `targets:` wins over harness preference and machine-local folder detection). Same project-scope flags as apply (`--project`, `--dry-run`, `--update`, `--force`, `--target`, `--all`, `--harness`). Does not take a plugin selector or `--global`. |
| `harnesstap compile` | Named entry over apply-from-manifest: same overlay writers, same `local_deployed_file_hashes`, same resolution chain as `ht install`. Does not take a plugin selector or `--global`. Fails closed when no target resolves. |
| `harnesstap targets` | Prints the canonical APM target table (or `--json`) for the current project: active/inactive, resolution source, deploy directory. Same resolution order as `ht compile` / apply-from-manifest. Not `ht harness list`. |
| `harnesstap mcp …` | Official MCP Registry v0.1 discovery and install. `mcp search` / `mcp list` / `mcp show` call `registry.modelcontextprotocol.io`. `mcp install <id>` is an alias of `ht install --mcp <id>`: append to `apm.yml` `dependencies.mcp`, then the existing apply-from-manifest install (including transitive `dependencies.apm`). Failed install rolls back the manifest write. Does not inject GitHub PATs, and does not add harness adapters HT does not already serialize. |
| `harnesstap lock export` | Inventory export from the existing `apm.lock.yaml` into CycloneDX 1.5 (default) or SPDX 2.3. Reads the lockfile only — never re-resolves, re-hashes, or touches the network. `--output` writes a file; default is stdout (diagnostics on stderr). `--timestamp` pins ISO 8601; otherwise `SOURCE_DATE_EPOCH`, then lockfile `generated_at`. `--project` selects the project directory; `--global` reads `~/.harnesstap/apm.lock.yaml` when that file already exists. Not a security attestation: unsigned, no SLSA claim. |
...
| `harnesstap audit` | Scans a project (or `--file`) for hidden Unicode and, with `--ci`, verifies lockfile SHA-256 hashes and evaluates `apm-policy.yml`. `--strip` removes critical and warning characters. `--require-policy` with `--ci` fails if no policy file exists. A pinned `policy.hash` on `apm.yml` fails closed when the policy file is missing or the digest does not match. Required-but-untrusted executables fail `--ci` with `required-executable-untrusted`. |
...
| `apply` | Applies plugin selectors, bundle paths (directory or `.zip`), or bundle URLs to a project; resolves environment cascade; serializes per platform; snapshots git-backed projects. With no selector, same loop as `ht install`: materializes `apm.yml` including git `dependencies.apm` (resolve → fetch exact SHA → lock; `--update` re-resolves) and compiles local `.apm/` (or root) primitives into resolved target harness dirs. Flags: `--strict-plugin-versions`, `--ignore-plugin-versions`, `--sync-plugins`, `--force` (override critical hidden Unicode), `--update` (refresh lock hashes and git refs), `--target` / `--all` / `--harness`. Packed bundles with `pack.bundle_files` are rehashed and fail closed on mismatch, extra, missing, or symlink. `plugin.json` is metadata and is never deployed as a harness file. Generated files are Unicode-scanned before write; lockfile `local_deployed_file_hashes` are verified on subsequent apply. `apm-policy.yml` is evaluated against the install plan before write. Opt-in `executables:` parks unapproved dependency hooks / bin / self-defined MCP and records lock `exec_status`. |
...
Repositories may declare named profiles, environments, and plugin composition in `apm.yml` at the repo root. Standard OpenAPM keys (`name`, `version`, `targets`, `dependencies.apm`, `dependencies.mcp`, `devDependencies`) parse as a vanilla APM manifest. Extra top-level keys are allowed and preserved (OpenAPM additionalProperties). HarnessTap-only fields are first-class top-level keys: `default_profile`, `environments`, `profiles`, and inline `plugins`. `environments` is a mapping whose `default` key names the active environment; other keys are named environments with `values` and optional secret **refs**. Vanilla APM readers ignore the extra keys. First-class `targets:` (list of canonical slugs: `copilot`, `claude`, `grok-build`, `cursor`, `opencode`, `codex`, `gemini`, `antigravity`, `windsurf`, `kiro`, `agent-skills`) pins portable compile/install ownership; `compilation.target` is used only when top-level `targets` / `target` are omitted. Optional `compilation` also accepts `exclude`, `strategy` (`distributed` is noted and ignored — output stays single-file root context), and `single_file`. Experimental APM targets that need a feature flag (`grok-cloud`, `hermes`, `intellij`, …) are not accepted. `ht config`, `ht use`, `ht install`, `ht apply`, `ht compile`, `ht targets`, and lock write/replay honor those fields. Git `dependencies.apm` entries (`owner/repo`, https/ssh/`git@`/`file://` URLs, optional `path` and `ref` / `#version`) resolve on `ht install` / `ht apply` to an exact commit; the lock records repo URL, SHA, constraint/ref, path, deployed-file hashes, and `declared_license` from the dependency manifest `license:` (never LICENSE file text). Fetched git / catalog / path packages are then walked for their own `dependencies.apm` (runtime deps only); each is pinned and deployed through the existing writers, with the same policy, Unicode, lock-hash, and executable-trust gates. Cycles, missing packages, and hash mismatches fail closed. The root manifest wins on package-name collisions. Subsequent apply without `--update` replays the locked SHA. `dependencies.mcp` registry strings (`io.github…` / MCP Registry v0.1) resolve on `ht install` / apply-from-manifest from `registry.modelcontextprotocol.io` into the same native MCP files HT already emits; self-defined `registry: false` stays inline. `ht lock export` serializes that lock into CycloneDX or SPDX. HarnessTap does not ship or invoke Microsoft's `apm` binary.
...
| `resource show` | Prints the resource with display labels. Origin is `Local` / `Local (/path)` (or Marketplace). Path is the package directory for plugins and `SKILL.md` packages. Plugin and `SKILL.md` package content lists nested files under that directory. Supports selector grammar. Desktop Library detail loads those files separately from metadata (`GET /v1/library/resources/:selector?include_contained=0`, then `GET /v1/library/resources/:selector/files?limit=&offset=`) so the page does not wait on the full tree. The Content list shows the first 20 files, then **Show more** / **Show all**. |
...
| `profile file-diff <name>` | Unified diff of a managed path: live on-disk content (`live/`) → after apply (`after-apply/`). For aggregate MCP configs, Desktop passes the selected `mcp_server` so the patch (and +/− counts) include only that server’s `mcpServers` / `mcp_servers` / `mcp` key. `--format json` returns `expected` and `current` without a rendered patch. |
...
**Scope:** `profile use` applies to machine **home** harness paths only. Project apply is `apply` (or `install` for the no-selector / `apm.yml` onboarding path).
...
`apply` resolves local plugin names, bundle paths (directory or `.zip`), and URLs. With no selector — the same path as `ht install` / `ht compile` — it compiles local `.apm/` primitives into harness directories (existing writers; `--target` / `--all` / `--harness`, then `targets` / `target` / `compilation.target`, then project/global harness preference, then filesystem signals). Install, compile, and apply-from-manifest fail closed when no target resolves. Packed bundles with `pack.bundle_files` are rehashed and fail closed on mismatch, extra, missing, or symlink. Subsequent apply of a project verifies `local_deployed_file_hashes` the same way unless `--update` is passed. Critical hidden Unicode in generated files blocks apply unless `--force` is passed. Published selectors (`org/catalog/name@version` or `org/name@version`) that are not installed locally are fetched from the catalog at apply time (same import path as `plugin pull`). `ht targets` previews the resolved apply set.
...
4. Seeds a local **`default` environment** when none exists and writes `active-environment.json` → `{ "name": "default" }` if the home active pointer is unset. Desktop `ht-agent` boot runs the same home scan and profile seed so a fresh install has a default home entry in tracked directories and a populated global default profile. Opening a project in the desktop app writes `apm.yml` if needed and seeds a **`project default`** profile from that repository’s on-disk resources. Desktop Project **Install** is `POST /v1/apply` with `plugins: []` (same as `ht install`). `POST /v1/apply` and successful project `/v1/switch` include `optedIn`, `parked`, `execStatuses`, and `warnings` from the executable trust gate so Desktop can show a pending-approvals strip. Settings → Project edits raw `apm.yml` (`GET`/`PUT /v1/config/raw`); save runs `validateProjectConfig` and refuses to write on errors, and hosts the per-project harness override (`GET`/`PUT /v1/harness` with the current global block echoed back). Desktop **Harnesses** reads `GET /v1/harness/inventory` (configured main + alias harnesses, disk presence `detected` / `shared-only` / `absent`, and per-location home resources including native global paths, host-managed paths, shared `~/.agents/` hubs, and related trees such as Cursor’s Claude plugin/skill directories) and writes membership through `PUT /v1/harness { global }`; it never runs global apply. **Sync harnesses** calls `POST /v1/harness/sync` (same as `ht harness sync`): union scan of the configured main + aliases, main-wins conflicts, shared skill path preference, existing serialize/materialize.
...
| `{project}/apm.yml` | Project manifest (OpenAPM keys plus first-class HarnessTap fields) for `ht use` / `ht install` / `ht apply` |
...
**Version cuts and dirty heads:** Each plugin name has a **working head** — the latest editable version. Edits after a cut mark the head **dirty** without changing its semver (human lists show `1.2.0*`; JSON keeps `version` plus `dirty: true`). `plugin cut --version <semver>` freezes the current composition (copy-on-write) and advances a clean head. `plugin versions` lists retained cuts. `plugin rollback --to` restores a frozen snapshot onto the working head without changing semver. Frozen versions cannot be edited or cut again. At most `layerVersionHistoryLimit` versions are retained per name (default `10` in `config.jsonc`); oldest frozen rows prune on cut. `migrate export --plugin` and `plugin publish` refuse dirty heads; pass `--version` on `plugin publish` to cut and publish in one step.
...
Library plugin resource detail includes `current_version`, `advertised_version` (from that marketplace's `marketplace.json`), and `available_versions` (host cache directories plus versions **Pull** fetched from the plugin git source). Desktop **Pull** (`POST /v1/library/resources/:selector/pull`) refreshes the marketplace checkout and lists every semver git tag on the plugin source (and the advertised catalog version). When the catalog `source` is a relative path such as `./` (the plugin *is* the marketplace git repo, as with `superpowers@superpowers-marketplace`), Pull lists tags on the marketplace remote. Selectors may be a resource ULID or the pin `origin_ref` (`name@marketplace`). Desktop shows Version as a filterable combobox. `POST /v1/library/resources/:selector/version` with `{ version }` downloads that version into the host cache when it is missing, retargets the Claude `installed_plugins.json` record for that origin ref, and runs `resource sync` with overwrite. Nested Content files are not required on the detail payload: `include_contained=0` skips the tree walk, and `GET /v1/library/resources/:selector/files` returns `{ files, has_more }` with `limit` / `offset` (Desktop default `limit=20`). Walks skip `.git` and `node_modules` directories.
...
Toolkit config (`~/.harnesstap/config.jsonc`) remains JSONC. The project manifest is `apm.yml` (OpenAPM YAML with first-class HarnessTap fields at the document root: `default_profile`, `environments`, `profiles`, `plugins`, `executables`). `environments.default` is the default/active environment name; other `environments` keys are named environments. Lockfiles use `apm.lock.yaml` (`lockfile_version: "1"`, APM dependency/MCP/file-hash fields including git identity `repo_url` / `resolved_commit` / `resolved_ref` / `constraint` / `virtual_path`, per-dep `exec_status` when the executable trust gate is on, plus root-level HT replay metadata: `root`, `resource_map_hash`, `environment` as the bound environment name, `plugins`). Packed bundles embed an enriched copy of that lockfile with `pack.bundle_files` (SHA-256 per packed file). User executable grants live under `executables.{allow,deny}` in `~/.harnesstap/config.jsonc`, not `~/.apm/config.json`.
...
Consumers install a packed directory or zip with `ht apply <path>` (or `migrate import`). When `pack.bundle_files` is present, every listed file is rehashed; mismatch, extra, missing, or symlink fails closed before import. `plugin.json` and `apm.lock.yaml` are bundle metadata and are never materialized as harness files. Apply of any project compiles local `.apm/` primitives into harness directories, scans generated files for hidden Unicode (critical findings block unless `--force`), and verifies `local_deployed_file_hashes` on subsequent apply (fail closed on mismatch, extra, or missing unless `--update`). `apm-policy.yml` is evaluated before write. The executable trust gate is opt-in (`executables:` on the project or a non-empty policy block). `ht audit` is the on-demand / CI scanner (Unicode, lock hashes, policy, and `required-executable-untrusted`).
...
The project uses Bun for local dependency management, CI, and builds. Distribution of the CLI is through the npm registry. An optional Tauri desktop app lives under `apps/desktop/` (`bun run desktop:dev` / `desktop:build` / `desktop:install`); it is not part of the published npm package. Tagged GitHub releases attach unsigned macOS, Windows, and Linux installers for x64 and arm where GitHub-hosted runners can build them. Linux AppImage, deb, and rpm artifacts are built on Ubuntu 22.04 so glibc stays compatible with Ubuntu 22.04 / Debian 12. Release packaging builds deb and rpm first, then AppImage. CI extracts linuxdeploy without FUSE and shelters `ht-agent` by sidecar name so linuxdeploy does not ldd or patchelf it. The Linux job fails if no AppImage is produced. macOS jobs ad-hoc reseal `HarnessTap.app` after the Tauri bundle so Sealed Resources / CodeResources exist (no Developer ID); the DMG is recreated from that sealed app.
...
- `ht compile` does not implement Microsoft APM's distributed per-directory `AGENTS.md` placement, `--watch`, `--global` compile, experimental targets (`grok-cloud`, `hermes`, `intellij`, …), signing, or SLSA. HarnessTap does not ship or invoke Microsoft's `apm` binary.
- MCP Registry install does not add a runtime sandbox, GitHub PAT auto-header injection, LSP, signing/SLSA, or VS Code / JetBrains / Windsurf / Kiro adapters HT does not already serialize. It does not walk `dependencies.apm` of fetched packages.
