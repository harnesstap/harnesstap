...
export interface EnvironmentDrawerProps {
  ...
  disabled?: boolean;
  onClose: () => void;
  onSaved: (message: string, name: string) => void;
}
...
function emptyEnvVars(): EnvVarRow[] {
  return [{ key: "", value: "" }];
}
...
export function EnvironmentDrawer({
  ...
  const [envVars, setEnvVars] = useState<EnvVarRow[]>(emptyEnvVars);
  ...
  useEffect(() => {
    ...
    void fetchEnvironment(baseUrl, token, environmentName)
      .then((payload) => {
        ...
        setEnvVars(vars.length > 0 ? vars.map(([key, value]) => ({ key, value })) : emptyEnvVars());
  ...
  const persistCreatedValues = async (
    created: EnvironmentShowPayload,
  ): Promise<void> => {
    if (!baseUrl) {
      return;
    }
    const extra = collectEnvVarMap(envVars);
    if (Object.keys(extra).length === 0) {
      return;
    }
    await putEnvironment(baseUrl, token, created.environment.name, {
      description: created.environment.description,
      env_vars: { ...created.values.env_vars, ...extra },
      model_configs: created.values.model_configs,
      permissions: created.values.permissions.map((row) => ({
        ...(row.name ? { name: row.name } : {}),
        action: (row.action === "deny" || row.action === "ask" ? row.action : "allow") as PermissionAction,
        pattern: row.pattern,
      })),
      secret_refs: Object.fromEntries(
        Object.entries(created.secret_refs).flatMap(([key, secret]) => {
          const provider = secret.provider === "keychain" || secret.provider === "file" || secret.provider === "env"
            ? secret.provider
            : null;
          return provider ? [[key, { provider, ref: secret.ref }] as const] : [];
        }),
      ),
    });
  };
  ...
  const runCreate = async () => {
    ...
    try {
      ...
      try {
        await persistCreatedValues(result.environment);
      } catch (saveError: unknown) {
        setError(errorMessage(saveError, "Created the environment, but could not save values"));
        onSaved(`Created environment ${name.trim()}`, name.trim());
        onClose();
        return;
      }
      onSaved(`Created environment ${name.trim()}`, name.trim());
  ...
  const runSave = async () => {
    ...
    setError(null);
    try {
      const env_vars = collectEnvVarMap(envVars);
      ...
      onSaved(`Updated environment ${environmentName}`, environmentName);
      onClose();
    } catch (saveError: unknown) {
      ...
      >
          ...
          {mode === "create" ? (
            <>
              ...
              <Collapse open={source === "from-plugin"}>
                <SelectionList
                  title="Plugins"
              ...
              >
                {envVars.map((row, index) => (
                  <div className="flex gap-2" key={`create-env-${index}`}>
                    <Input
                      placeholder="KEY"
                      value={row.key}
                      disabled={controlsDisabled}
              ...
              <p className="field-note muted">
                Secret refs can be added after you create this environment.
              </p>
