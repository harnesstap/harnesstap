...
export function ProjectConfigInspect({
  ...
  async function handleSave(): Promise<void> {
    ...
    try {
      ...
      toast({ tone: "success", title: "Saved apm.yml" });
    } catch (error: unknown) {
      ...
      {hasRaw && !loading ? (
        <div className="form-field" data-testid="project-config-yaml">
          ...
          <ApmYamlEditor
            ...
            invalid={saveErrors.length > 0 || validation?.valid === false}
            errors={editorErrors}
            onChange={(next) => {
              setDraft(next);
              setSaveErrors([]);
...
function ApmYamlEditor({
  ...
  const errorLines = useMemo(() => {
    ...
    for (const message of errors) {
      const line = parseValidationLineNumber(message);
      if (line) {
        lines.add(line);
      }
    }
  ...
  return (
    <div className="apm-yml-editor">
      <div className="apm-yml-gutter" ref={gutterRef} aria-hidden="true">
        {Array.from({ length: lineCount }, (_, index) => {
          const line = index + 1;
          return (
            <div
              key={line}
              className={["apm-yml-line", errorLines.has(line) ? "is-error" : ""]
                .filter(Boolean)
                .join(" ")}
            >
              {line}
      ...
      <textarea
        ...
        className="apm-yml-textarea mono"
        value={value}
        rows={15}
        disabled={disabled}
        spellCheck={false}
