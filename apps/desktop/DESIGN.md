...
- **Remove profile** is an icon-only control in the edit-profile header (accessible name and tooltip **Remove profile**), next to Done. Confirm dialog keeps the delete-plugin checkbox. The same labeled **Remove profile** control appears on the live-state header when a profile is selected.
