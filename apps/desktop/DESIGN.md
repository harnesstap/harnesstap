...
- Library type chips (sidebar Type filters): compact outline pills — 10px type, tight padding, transparent fill, selected uses an 8% accent wash (not a heavy filled pill). Zero-count types are omitted unless selected.
...
- Filters: type as compact outline badges with human labels (Plugins, Skills, MCP, Plugin refs — not `plugin` / `mcp_server`); updated as a segment; namespace as a searchable select; origin as a radio list. Origin groups `manual` and `local_snapshot` as **Local**; `marketplace_link` as **Marketplace**. Same labels on hover and detail. Storage `origin_kind` is unchanged. Filter placeholder is **Filter by name** (typed `type:name` queries still work). List section headings use the same human labels (Plugins / Skills), with the count beside the heading. **Clear filters** restores every facet to its default (All / empty). Type chips omit zero-count types unless that type is selected.
