...
- Header cluster (right of the Library title): icon-only **Create resource** (accent), **Import**, **Tracked directories**, **Update all**, each with a hover tooltip of that label. Create resource opens the centered type-picker dialog, then a full-screen single-page form per type with Cancel/Create in the footer; canceling dirty edits confirms discard. Material forms offer an optional add-to-current-profile-and-apply checkbox. Plugin create **Compose from library** reuses the profile Resources picker (filter + collapsible type groups with counts). **Update all** is disabled when origin check found no outdated plugin-package heads or an update is in flight. Confirm: “Update N plugins from origin?”
