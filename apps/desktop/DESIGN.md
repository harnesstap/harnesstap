...
- Labeled `btn primary` for dialog confirm actions (Create, Publish, Save, Update) together with an icon. Profiles rail **Apply** / **Re-apply** is a full-width accent `btn primary` with visible text and the icon to the right of the label. Library record primary actions that are not square icon chrome show a short visible label beside the icon: plugin **Apply**, resource **Sync** / **Write**, frozen **Restore**. Other persistent page chrome — Clear, Remove, Add all, More, Show all, Install, Library/Sources header actions, and secondary/destructive record actions — is icon-only with a hover tooltip that keeps the original label. Header destinations show icon plus name.
