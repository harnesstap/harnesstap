...
- **Remove profile** is an icon-only trash control in the edit-profile header (accessible name **Remove profile**; tooltip **Remove profile**). Disk files stay until you apply another profile (confirm dialog). Next to Done, not in the Profiles rail next to create **+**. Create vs remove must not be confused: rail **+** is accent **Create a new profile**; trash is never in that cluster. Confirm dialog keeps the delete-plugin checkbox. The same control appears on the live-state header when a profile is selected. A ListPlus on an active row adds not-staged items (tooltip **Add N not-staged items to this profile**, not create, not remove).
