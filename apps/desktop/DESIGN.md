...
- ResourceTypeTabs: presence-filtered pill/segmented type switcher (selected filled `--accent`, others muted outline). Hide empty types. Show **All** only when two or more types are present. Wide (≥~900px container) icon plus label; narrow icon-only with a Radix tooltip plus `aria-label`. Hit target ≥32px. Overflow is horizontal scroll with an edge fade. Icons use `TypeIcon` / `resourceTypeGlyph`. Sparkles is skill only. All uses LayoutGrid. Short labels: Plugins, MCPs, Skills, Subagents, Rules, Commands, Hooks, Instructions, Permissions, Env vars, Model config, Plugin refs.
