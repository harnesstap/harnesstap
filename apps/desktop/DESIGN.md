...
- Library Sync = `resource sync` (refresh the library copy from its install source). Never means Apply. The dry-run control is labeled **Sync**; committing that preview overwrites the library copy and is labeled **Write**, not Apply.
...
- Resource detail (skill, MCP, plugin ref, and other library material): labeled **Sync** previews `resource sync` from the install source; labeled **Write** overwrites that resource in the library with the pending sync. Neither applies a parent plugin (or the resource) to a project or host. Secondary **Delete** stays icon-only.
