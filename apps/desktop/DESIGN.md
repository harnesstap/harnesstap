...
- **Remove profile** is an icon-only trash control in the edit-profile header (accessible name **Remove profile**; tooltip explains files on disk stay), next to Done — not in the Profiles rail next to create **+**. Confirm dialog keeps the delete-plugin checkbox. The same control appears on the live-state header when a profile is selected. Rail **+** is **Create a new profile**; a ListPlus on an active row adds not-staged items (not create, not remove).
