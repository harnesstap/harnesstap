...
describe("desktop icon chrome", () => {
  ...
  test("instruction, plugin, and package rows never map to or render Sparkles", () => {
    ...
    expect(typeTabsSource).toContain("aria-label={ariaLabel}");
  ...
  test("file change rows use a kind chip plus icon actions, not Apply to write", () => {
    ...
    expect(liveStateSource).toContain("function TargetPreviewTypeBadges");
    expect(liveStateSource).toContain("apply-diff-type-badge");
    expect(stylesSource).toContain(".apply-diff-type-badge");
