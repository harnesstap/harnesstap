...
import { applyClaudeLayerExtensions } from "../../src/platforms/claude-layer-extensions.ts";

describe("applyClaudeLayerExtensions", () => {
  it("merges marketplaces and plugins into settings.json", () => {
    const projectDir = createTempDir("claude-layer-ext");

    const files = applyClaudeLayerExtensions(
      [],
      {
        marketplaces: {
          "team-plugins": {
  ...
  it("preserves existing settings read from disk", () => {
    const projectDir = createTempDir("claude-layer-ext-merge");
  ...
  it("merges with generated settings from resources", () => {
    const projectDir = createTempDir("claude-layer-ext-resource");
