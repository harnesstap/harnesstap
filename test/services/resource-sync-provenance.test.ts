...
import { LayerProvenanceError } from "../../src/services/layer-origin.ts";
...
describe("sync provenance gating", () => {
  it("refuses syncing a local dependency that names an authored layer", async () => {
    createLayer({ name: "mine", version: "1.0.0" });
  ...
  it("does not refuse marketplace deps just because a consumer layer is authored", async () => {
    const root = createLayer({ name: "mine" });
