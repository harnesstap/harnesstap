...
import { setLayerVersionOverride } from "../../../src/services/layer-overrides.ts";
...
async function dependOn(
  ...
  const parent = getLayerByName(parentName);
  if (!parent) throw new Error(`missing layer ${parentName}`);
  await addLayerAttachment({
    layer: parent,
    selector: `layer:${depName}`,
    ...(version ? { version } : {}),
  });
}
...
describe("walkDependencyGraph", () => {
  it("unifies a diamond to one version satisfying both constraints", async () => {
    ...
    createLayer({ name: "left", version: "1.0.0" });
    createLayer({ name: "right", version: "1.0.0" });
    ...
    const walk = walkDependencyGraph({ rootLayerId: root.id });
  ...
  it("records the shortest depth when a name is reachable by two paths", async () => {
    createLayer({ name: "base", version: "1.0.0" });
    createLayer({ name: "mid", version: "1.0.0" });
    const root = createLayer({ name: "root", version: "1.0.0" });
  ...
  it("lets a root override beat a transitive constraint", async () => {
    ...
    await dependOn("root", "mid");
    setLayerVersionOverride(root.id, "base", "1.0.0");
  ...
  it("errors with both requirers when the intersection is empty", async () => {
    createLayer({ name: "base", version: "1.4.0" });
    createLayer({ name: "base", version: "2.1.0" });
    createLayer({ name: "team-standards", version: "2.1.0" });
    createLayer({ name: "legacy-review", version: "1.4.0" });
    const root = createLayer({ name: "my-setup", version: "1.0.0" });
    ...
    expect(() => walkDependencyGraph({ rootLayerId: root.id })).toThrow(
