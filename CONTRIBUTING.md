...
3. Review and merge the release PR (updates `CHANGELOG.md`, root `package.json`, and Desktop version stamps in `apps/desktop/package.json`, `apps/desktop/src-tauri/tauri.conf.json`, and `apps/desktop/src-tauri/Cargo.toml` so installer filenames match the CLI/Changie version).
